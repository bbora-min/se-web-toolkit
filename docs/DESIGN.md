...
```
- `@se/tokens`의 `createTheme(identity)`가 CSS 변수 세트를 생성. 대비(WCAG AA)와 의미 색 hue 거리는 빌드 시 검증되어 실패하면 빌드 실패.
...
| T2 레이아웃/패턴 | AppShell(`layout` = sidebar · topnav · panes), ListDetailPage, DashboardPage, DetailPage, FormWizardPage, SettingsPage, **HubPage**(0.11.0, `examples/se-home` — 패턴은 컴포넌트가 아니라 복사할 원본) — 관측 벽·보드·문서 골격은 0.12.0+ |
...
- **문서**: Storybook(사람용, `apps/storybook`) = 스킬 레퍼런스와 같은 TSDoc 이 원본. "가족 초상화" 스토리가 레지스트리의 서비스 전부를 나란히 — 툴바의 아이덴티티·테마·밀도가 곧 슬롯이다. main 은 GitHub Pages 로 배포.
...
| P3 배포 **(완료)** | Storybook·아이덴티티 갤러리(0.7.0) · 스크린샷 기준 비교 CI · React 18 CI 레인 · codemods(0.8.0) · 자동 업그레이드 PR + `@se/upgrade` CLI · npm publish 워크플로(0.9.0, 저장소 설정으로 켬) | 웹 비전공 구성원이 혼자 서비스 1개 출시 |
...
- 컴포넌트 변경은 레퍼런스 앱 3개에서 라이트·다크·1024 스크린샷으로 확인한 뒤 merge 한다(한 컴포넌트가 세 서비스 모두에서 괜찮은가). CI `visual` job 이 세 앱의 화면 16개 × 3(라이트 1280·다크 1280·라이트 1024)을 **기준 스크린샷과 비교**한다(`pnpm visual`, 시계 고정·시드 목 데이터, 허용 오차 1 %). 의도한 변화면 Actions › `visual-baseline` 로 기준을 CI 러너에서 다시 찍는다 — "화면이 바뀌었는데 patch" 를 기계가 잡는다.
