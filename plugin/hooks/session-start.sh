...
sub=""
# 앱이 하위 폴더(frontend/ 등)에 있는 모노 구조도 잡는다 — 한 단계만
if ! grep -q '"@se/ui"' "$root/package.json" 2>/dev/null; then
  found="$(grep -l '"@se/ui"' "$root"/*/package.json 2>/dev/null | head -n1)"
  [ -n "$found" ] || exit 0
  root="${found%/package.json}"
  sub=" (${root##*/}/)"
fi
...
ui_v="$(node -e "for(const d of ['$root','$top']){try{console.log(require(d+'/node_modules/@se/ui/package.json').version);process.exit(0)}catch{}}console.log('미설치')" 2>/dev/null)"
identity=""
...
{
  echo "[SE Web Toolkit] @se/ui $ui_v$sub · Node $node_v"
