...
| **4 페이지** | 트래픽 많은 페이지부터 하나씩 `se-migrator` 에이전트로 치환(Button·Input·DataTable·Dialog·토큰 색). 전/후 스크린샷 비교. 못 옮기는 건 `// TODO(se-adopt)`. 마지막 커밋에서 2단계 공존 블록·레거시 CSS·미사용 컴포넌트를 지워 CSS 진입 파일을 템플릿과 같은 5줄로. 툴킷이 못 하는 것(컴포넌트 prop 부재 등)은 리포트 "툴킷에 제안할 것" 에 모은다 | 페이지별 PR |
