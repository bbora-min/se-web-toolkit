...
- 쉘 배치(`shell`)는 서비스의 골격이다: 목록·대시보드 도구 → `sidebar`, 입구·콘솔처럼 폭이 필요한 도구 → `topnav`, 처리함·로그처럼 화면 안에서 패널을 나누는 도구 → `panes`. **형제가 이미 둘 이상 쓰는 배치는 피한다** — 레지스트리 검사가 셋째부터 경고한다. 색이 달라도 배치가 같으면 "색만 다른 형제"다
