...
`se.identity.json`의 슬롯 9개(액센트 hue · 뉴트럴 편향 · 마크 · **쉘 배치** · 시그니처 · 밀도 · 헤딩 폰트 · 차트 · 톤)를 정한다. 타이포·간격·컴포넌트 해부학과 쉘의 자리(로크업·⌘K·테마 토글)는 고정이라 여기서 바꾸지 않는다.
...
- 쉘 배치(`shell`)는 서비스의 골격이다: 목록·대시보드 도구 → `sidebar`, 입구·콘솔처럼 폭이 필요한 도구 → `topnav`, 트리아지·로그처럼 화면 안에서 패널을 나누는 도구 → `panes`. **형제가 이미 둘 이상 쓰는 배치는 피한다** — 레지스트리 검사가 셋째부터 경고한다. 색이 달라도 배치가 같으면 "색만 다른 형제"다
- 시그니처는 화면의 첫 줄이다. 서비스의 핵심 질문에 맞춘다: "지금 괜찮은가" status-strip · "무엇을 찾나" search-hero · "어디까지 왔나" stage-rail · "최근 무슨 일이" timeline-ribbon · "얼마인가" metric-marquee. 형제가 이미 쓰는 시그니처는 피한다(레지스트리)
