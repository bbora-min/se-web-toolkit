...
2. 스크린샷: `pnpm e2e`(`e2e/screens.spec.ts`의 목록 × light-1440·dark-1440·light-1024) 또는 `$ARGUMENTS`의 라우트를 Playwright로. 산출물 경로를 모은다. **에러 화면은 "다시 시도" 버튼이 보이는지 확인** — 로딩 화면을 에러로 착각하지 않는다
...
7. `docs/design-review.md`에 날짜·점수·반영 내역을 덧붙인다. **앱에서 못 고치는 결함은 "툴킷에 제안할 것"** 절로 분리한다 — 사용자가 툴킷 저장소에 이슈/PR로 옮긴다
8. 결함을 고친 뒤에는 반드시 **재채점**해 마지막 점수를 기록한다. "고쳤으니 올랐을 것"으로 끝내지 않는다
