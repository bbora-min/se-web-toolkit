// 원본: examples/reference-app/src/pages/jobs/JobConsolePage.tsx (자동 복사 — 수정하지 말 것, pnpm gen:skill-docs)
/**
...
import { Badge, Button, ErrorState, FacetGroup, Input, LogViewer, ShellFill, Skeleton, SplitPane, StatusBadge, Switch, formatAbsolute, formatDuration, formatRelative, stripAnsi, toast, useContentWidth } from '@se/ui'
...
const LEVEL_RE = /\b(ERROR|FATAL|PANIC)\b|\b(WARN|WARNING)\b/
...
function parse(lines: string[]): Parsed[] {
  ...
  return lines.map((raw) => {
    const p = stripAnsi(raw)
    const lv = LEVEL_RE.exec(p)
    const level: Level = lv?.[1] ? 'ERROR' : lv?.[2] ? 'WARN' : 'INFO'
    const st = STAGE_RE.exec(p)
    const found = st?.[1] ?? st?.[2]
    if (found) stage = found.trim()
    return { raw, level, stage }
  })
}
...
export function JobConsolePage() {
  ...
  const j = job.data
  const logs = useJobLogs(j?.id, j?.state === 'running')
  const siblings = useJobs({ pipeline: j?.pipeline ?? '', pageSize: 6 })
  ...
  const filtered = React.useMemo(() => {
    ...
    return parsed.filter((l) => (!levels.length || levels.includes(l.level)) && (!stages.length || stages.includes(l.stage)) && (!needle || stripAnsi(l.raw).toLowerCase().includes(needle))).map((l) => l.raw)
  }, [parsed, levels, stages, q])
  const count = (pick: (l: Parsed) => string) => {
    const m = new Map<string, number>()
    for (const l of parsed) m.set(pick(l), (m.get(pick(l)) ?? 0) + 1)
    return m
  }
  const byLevel = count((l) => l.level)
  const byStage = count((l) => l.stage)
        ...
        <SplitPane
          ...
          left={
            <div className="flex flex-col gap-4 px-2 py-3">
              ...
              {j ? <FacetGroup title="시도" values={[]} onChange={() => {}} options={Array.from({ length: j.attempts }, (_, i) => ({ value: `#${i + 1}`, label: `${i + 1}회${i + 1 === j.attempts ? ' (현재)' : ''}`, count: i + 1 === j.attempts ? parsed.length : undefined }))} /> : null}
            </div>
          }
        ...
        >
          ...
          <div className="flex h-11 shrink-0 items-center gap-3 border-b border-line bg-surface px-3">
            ...
            <label className="ml-auto flex shrink-0 items-center gap-2 whitespace-nowrap text-xs text-muted">
              라이브 테일
              <Switch checked={live} onCheckedChange={setLive} aria-label="라이브 테일" disabled={j?.state !== 'running'} />
            </label>
          </div>
          <div className="flex min-h-0 flex-1 flex-col p-3">
            {logs.isPending ? (
              ...
              <LogViewer
                lines={filtered}
                live={Boolean(live && logs.data?.live)}
                height="100%"
