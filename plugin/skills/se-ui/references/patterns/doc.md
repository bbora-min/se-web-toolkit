...
"읽는 화면"이다. 런북·포스트모템·온콜 핸드북·ADR·데이터 사전처럼 사람이 위에서 아래로 읽는 것. 원장(스캔)·관측 벽(훑기)·보드(흐름)와 달리 **타이포와 여백이 전부**고 크롬(제목 바·필터·표 헤더)이 없다. 같은 앱 안에 있어도 이 화면만은 다르게 보여야 한다.
...
- **데이터 사전은 표지만 `DataTable` 이 아니다** — `DataTable` 은 정렬·페이지가 있는 원장용. 문서 속 표는 정적 `Table` 프리미티브(`Table`·`TableHeader`·`TableRow`·`TableHead`·`TableCell`)를 `se-prose-table` 로 감싼다
