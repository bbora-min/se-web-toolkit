...
"고르는 화면"이다. 목록(원장)이 아니라 카탈로그 — 항목이 수십 개 이하이고 하나를 골라 **다른 곳으로 가는** 것이 목적일 때. 서비스 포털·도구 모음·문서 허브·팀 홈.
...
<PageBody gap-8>
  ...
  <Button secondary h-12 w-full onClick={useShellSearch()}>  ← 큰 검색. 헤더의 ⌘K 와 같은 팔레트를 연다 — 장식 검색창 금지
  [빠른 진입 칩 3–5개 — 숫자 있는 것만]
  <시그니처 />                                      ← 가족 전체의 24시간(TimelineRibbon, 줄 = 항목) 또는 서비스의 것
  <section>
    <SectionHead title note link="표로 보기" />
...
## 결정 규칙
- **카드 vs 표**: 고를 땐 카드(홈), 비교할 땐 표(`/services`, 원장). 둘 다 두고 "표로 보기" 링크로 잇는다
...
- empty: "아직 등록된 서비스가 없어요" + `/se:new` 안내 버튼
