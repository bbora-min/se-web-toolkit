# TriagePage — 트리아지 · 목록 | 본문 | 속성

원본: `references/examples/release-desk/ApprovalsPage.tsx`(내 승인 대기 — 3단, j/k, 결정 뒤 다음 항목)

"하나씩 처리하는" 화면이다. 받은 편지함(Gmail·Superhuman)·이슈 트리아지(Linear·Sentry)·승인 대기. 원장은 표에서 고르고 상세로 *가지만*, 트리아지는 상세가 옆에 *상주*하고 처리하면 다음으로 넘어간다. 화면은 세로로 스크롤하지 않는다 — 칸마다 스크롤한다.
...
- 목록이 1만 건이면 서버 페이지네이션 + 가상 스크롤. 트리아지는 보통 수십 건 — "내 것"으로 좁힌다
