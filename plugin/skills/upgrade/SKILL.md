...
## 절차
1. **현재 ↔ 목표 + 할 일**: 기계 부분은 CLI 가 한다 —
   `pnpm dlx "github:bbora-min/se-web-toolkit#path:packages/upgrade" --dir . --dry` (또는 `--to $ARGUMENTS`)
   현재 태그(`#path:` 만 있으면 "main 추적" — 이번에 태그로 고정), 목표 태그, 그 사이 CHANGELOG 의 "앱에서 할 일" 을 출력한다. 목록을 사용자에게 먼저 보여 준다. major 가 끼어 있으면 여기서 멈추고 확인받는다
2. (1 과 같은 출력에 포함)
...
4. **갱신 + 기계 검사**: 같은 CLI 를 `--dry` 없이 — 참조를 `github:…#<태그>&path:packages/<pkg>` 로 바꾸고 `pnpm install` → typecheck → lint 까지 돌린다 (`pnpm update` 는 태그를 넘어가지 않는다 — 참조 자체를 바꿔야 한다)
5. major 의 "앱에서 할 일" 은 여기서 오류로 드러난다 — 하나씩 고치고, 못 고치는 건 `// TODO(se-upgrade)`
...
- 툴킷 리포의 `upgrade-apps` 워크플로가 이미 `se-upgrade/<태그>` 브랜치와 PR 을 열어 뒀을 수 있다(레지스트리에 repo 가 적힌 앱). 그러면 그 브랜치에서 3단계(스크린샷)부터 이어서 한다
